...
public class AddNewGroup extends AncestorEntryPoint {
	...
	bttCreate.addClickHandler(new ClickHandler() {
		...
		if (name != "") {
		    ...
		    panelValidateGroupName.add(new HTML("<div class=\"error-left\"></div><div class=\"error-inner\">" + name + "</div>"));
		    panelValidateGroupDescription.setVisible(false);
		    panelValidateGroupName.setVisible(true);
		    return;
		} else if (groupdes != "") {
		    panelValidateGroupDescription.clear();
		    panelValidateGroupDescription.add(new HTML("<div class=\"error-left\"></div><div class=\"error-inner\">" + groupdes + "</div>"));
		    panelValidateGroupName.setVisible(false);
	...
	@Override
	public void onClick(ClickEvent event) {
	    Window.Location.replace(HTMLControl.trimHashPart(Window.Location.getHref()) + HTMLControl.HTML_GROUP_MANAGEMENT_NAME);
	}
    }
