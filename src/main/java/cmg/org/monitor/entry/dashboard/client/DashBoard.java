...
public class DashBoard implements EntryPoint {
		...
		if (result != null) {
			...
			for (int i = 0; i < result.length; i++) {
				...
				dataListSystem.setValue(
						i,
						6,
						HTMLControl.getHTMLStatusImage(result[i].getStatus()
								&& result[i].isActive()));
				dataListSystem.setValue(i, 7,
						(result[i].isActive() ? HTMLControl.HTML_ACTIVE_IMAGE
								: ""));
			}
			...
			initMessage("No system found. ", "AddNewSystem.html",
		...
		timerMess = new Timer() {
			...
			public void run() {
				initMessage(
						...
						"DashBoard.html", "Reload now.", typeMessage);
	...
	void initMessage(String message, String url, String titleUrl, int type) {
		...
		RootPanel.get("content-" + HTMLControl.getColor(type)).add(
				new HTML(message + " <a href=\"" + url + "\">" + titleUrl
						+ "</a>", true));
	}
