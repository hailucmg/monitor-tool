...
public class XMLMonitorParser {
	 ...
	 * @return
	 */
	public List<JVMMemoryDto> getJVMComponent(String xmlContent) {
		List<JVMMemoryDto> jvmMems = new ArrayList<JVMMemoryDto>();
		try {
			...
			JVMMemoryDto jvmDto = null;
			Element element = null;
			if (totalElements > 0) {
				for (int i = 0; i < totalElements; i++) {
				  element = (Element) elementList.item(i);
				  jvmDto = new JVMMemoryDto();
			      jvmDto = xmlToJVM(jvmDto, element);
			      jvmMems.add(jvmDto);
				}
			}
		} catch (Throwable t) {
	 ...
	 * @return
	 */
	public List<CpuDto> getOriginalCPU(String xmlContent) {
	 ...
	 * @return
	 */
	public List<FileSystemDto> getFileSystem(String xmlContent) {
		List<FileSystemDto> fileSystems = new ArrayList<FileSystemDto>();
		try {
			...
			FileSystemDto jvmDto = null;
			Element element = null;
			if (totalElements > 0) {
				for (int i = 0; i < totalElements; i++) {
				  element = (Element) elementList.item(i);
				  jvmDto = new FileSystemDto();
	 ...
	 * @return
	 */
	public List<CpuPhysicalDto> getPhysicalCPU(String xmlContent) {
		List<CpuPhysicalDto> physicals = new ArrayList<CpuPhysicalDto>();
		try {
			...
			CpuPhysicalDto physicalCpuDto = null;
			Element element = null;
			if (totalElements > 0) {
				for (int i = 0; i < totalElements; i++) {
				  element = (Element) elementList.item(i);
				  physicalCpuDto = new CpuPhysicalDto();
	...
	private JVMMemoryDto xmlToJVM(JVMMemoryDto jvmDto, Element element) {
	...
	private FileSystemDto toFileSystem(FileSystemDto cpuDto, Element element) {
        ...
        if (fileName!= null && fileName.getLength() > 0) {
	        ...
	        cpuDto.setName(getCharacterDataFromElement(nameElement));
        }
        ...
        if (available!= null && available.getLength() > 0) {
	        ...
	        cpuDto.setAvailable(Long.parseLong(Ultility.extractDigit(getCharacterDataFromElement(nameElement))));
        }
        ...
        if (percent!= null && percent.getLength() > 0) {
        	...
        	cpuDto.setPercentUsed(Integer.parseInt(Ultility.extractDigit(getCharacterDataFromElement(nameElement))));
        }
        
        NodeList mount = element.getElementsByTagName("mount");
        if (mount!= null && mount.getLength() > 0) {
        	Element nameElement = (Element) mount.item(0);
	        cpuDto.setMount(getCharacterDataFromElement(nameElement));
        }
	...
	private CpuPhysicalDto toCPUPhysical(CpuPhysicalDto cpuDto, Element element) {
        ...
        if (used!= null && used.getLength() > 0) {
        	Element nameElement = (Element) used.item(0);
	        cpuDto.setTotal(Double.parseDouble(Ultility.extractDigit(getCharacterDataFromElement(nameElement))));
        }
        
        NodeList available = element.getElementsByTagName("used");
        if (available!= null && available.getLength() > 0) {
        	Element nameElement = (Element) available.item(0);
	        cpuDto.setUsed(Double.parseDouble(Ultility.extractDigit(getCharacterDataFromElement(nameElement))));
        }
