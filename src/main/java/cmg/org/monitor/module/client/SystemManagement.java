...
public class SystemManagement extends AncestorEntryPoint {
	...
	if (currentPage == HTMLControl.PAGE_SYSTEM_MANAGEMENT) {
	    ...
	    tableLinkDefault.getCellFormatter().setVerticalAlignment(0, 0, HasVerticalAlignment.ALIGN_MIDDLE);
	    tableLinkDefault.getCellFormatter().setVerticalAlignment(0, 1, HasVerticalAlignment.ALIGN_MIDDLE);
	    tableLinkDefault.getCellFormatter().setVerticalAlignment(0, 2, HasVerticalAlignment.ALIGN_MIDDLE);
	...
	popupContent.setHTML("<h4>Do you want to delete System ID " + code + "?</h4>");
	...
	monitorGwtSv.listSystems(new AsyncCallback<SystemMonitor[]>() {
		...
		showMessage("Server error. ", HTMLControl.HTML_SYSTEM_MANAGEMENT_NAME, "Try again.", HTMLControl.RED_MESSAGE, true);
		setOnload(false);
	    }
	});
	monitorGwtSv.getLink(new AsyncCallback<String>() {
		...
		showMessage("Can not load default link.", "", "", HTMLControl.RED_MESSAGE, true);
	    }
	...
	monitorGwtSv.deleteSystem(sysID, new AsyncCallback<Boolean>() {
		...
		showMessage("System deleted sucessfully.", "", "", HTMLControl.BLUE_MESSAGE, true);
		dialogBox.hide();
		initContent();
	    }
	    ...
	    @Override
	    public void onFailure(Throwable caught) {
		showMessage("Cannot delete this system.", "", "", HTMLControl.RED_MESSAGE, true);
		dialogBox.hide();
	    }
	});
	...
	for (int i = 0; i < result.length; i++) {
	    dataListSystem.setValue(i, 0, HTMLControl.getLinkEditSystem(result[i].getId(), result[i].getCode()));
	    dataListSystem.setValue(i, 1, result[i].getName());
	    dataListSystem.setValue(i, 2, "<a href=\"" + result[i].getUrl() + "\"" + " target=\"_blank\">" + result[i].getUrl() + "</a>");
	    dataListSystem.setValue(i, 3, result[i].getIp());
	    dataListSystem.setValue(i, 4, HTMLControl.getHTMLStatusImage(result[i].getId(), result[i].getHealthStatus()));
	    dataListSystem.setValue(i, 5, HTMLControl.getHTMLActiveImage(result[i].isActive()));
	    dataListSystem.setValue(i, 6, "<a onClick=\"javascript:showConfirmDialogBox('" + result[i].getCode() + "','" + result[i].getId()
	...
	monitorGwtSv.editLink(link, new AsyncCallback<Void>() {
	    ...
	    public void onFailure(Throwable caught) {
		// TODO Auto-generated method stub 
		showMessage("Updated Failed.", "", "", HTMLControl.RED_MESSAGE, true);
	    }

	    @Override
	    public void onSuccess(Void result) {
		// TODO Auto-generated method stub 
		showMessage("Updated sucessfully.", "", "", HTMLControl.BLUE_MESSAGE, true);
	    }	
	});
    }
}
