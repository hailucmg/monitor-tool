...
/**
 * 
 * @author Google Gdata and lamphan
 * 
 ...
 */
public class EmailDomainClientApps {

  private static final Logger LOGGER = 
      Logger.getLogger(EmailDomainClientApps.class.getName());
  
  // Number of email messages to insert.
  private static final int ITEMS_TO_BATCH = 1;
  
  // The mail message to migrate.
//  private static String rfcTxt =
//    "Received: by 10.143.160.15 with HTTP; Mon, 16 Nov 2011 10:12:26 -0700 (P"
//    + "DT)\r\n"
...
//    + "\r\n"
//    + "This is a message delivered via the Email Monitor System\r\n"
//    + "\r\n"; 
  ...
  private static String rfcTxt =
		    ...
		    + "From: \"System Monitor\" <monitor@c-mg.vn>\r\n"
		    + "To: \"Monitor Group\" <monitor.globe@c-mg.vn>\r\n"
		    + "Subject: Subject \r\n" 
		    + "MIME-Version: 1.0\r\n"
		    + "Content-Type: text/plain; charset=ISO-8859-1; format=flowed\r\n"
  ...
  private static String lastRfcTxt =
			  "This is a error message delivered via the Email Monitor System\r\n"
			  + "\r\n"; 
  
  private static final String MONITOR_LABEL = "Monitor Alert";
  ...
  private void doMail(String rfcContent) throws MonitorException {
    ...
    for (int i = 0; i < entries.length; i++) {
      entries[i] = setupMailItem(rfcContent, properties, labels);
    }
  ...
  private MailItemEntry setupMailItem(String rfcText,
    ...
    String randomFactor = Integer.toString(10000 + 
        (new Random()).nextInt(90000));
    rfcText = rfcText.replace("Subject: Subject",
        "Subject: Monitor Alert " + randomFactor);
  ...
  public static void main(String[] arg) throws Exception {
	...
	contentRtc.append(MailService.buildDateRfc()).append(rfcTxt).append(lastRfcTxt);
    try {
    	Appforyourdomain client = new Appforyourdomain("monitor@c-mg.vn",
    			  "31102011", "c-mg.vn"); 
		String[] emailAddresses =  client.listAllUser("monitor.globe@c-mg.vn");
		StringBuffer emailID = new StringBuffer();
		for (String user : emailAddresses) {
			for (int i = 0; i < user.length(); i++) {
				if (user.charAt(i)== '@') {
					emailID.append(user.substring(0, i));
					break;
				}
			}
			
			new EmailDomainClientApps("monitor", "31102011", "c-mg.vn", "lam.phan", contentRtc.toString());
		}
	} catch(Throwable e)	{
	}	
  }
